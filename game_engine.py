...
class GameEngine:
    ...
    def find_animal(self, name: str) -> Optional[Leaf]:
        """ищет лист с заданным именем животного"""
        name_lower = name.lower()
        stack = [self.root]  # создаёт стек для обхода в глубину, начиная с корня
        while stack:
            node = stack.pop()
            if isinstance(node, Leaf):
                if node.animal.lower() == name_lower:
                    return node  # возвращает найденный лист
            else:
                stack.extend([node.no, node.yes])  # добавляет в стек обе ветки (порядок не важен)
        return None